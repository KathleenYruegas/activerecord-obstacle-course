...
describe 'ActiveRecord Obstacle Course' do
  ...
  it "groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    grouped_items = order.items.group(:name).order(:name)
    # ------------------------------------------------------------
  ...
  it 'gets all item names associated with all orders' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    names = Order.joins(:items).select("items.name").pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.select("users.name, order_items.item_id")
            .where("order_items.item_id = ?", item_8.id)
            .joins(:orders)
            .joins(:order_items)
  ...
  it 'returns all orders which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.select("orders.id, order_items.item_id")
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.select("orders.user_id, orders.id, order_items.item_id")
                  .where("order_items.item_id = ?", item_4.id)
                  .where(user_id: 2)
                  .joins(:order_items)
                  .order(:id)
  ...
  it 'returns items that are associated with one or more orders' do
    ...
    ordered_items = Item.select("items.*, order_items.item_id")
      .joins(:order_items)
      .order(:id).distinct
  ...
  it 'returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.select("items.*, order_items.item_id")
            .joins(:order_items)
            .order(:id).distinct
            .pluck(:name)
    # ---------------------------------------------------------------
